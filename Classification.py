...
from data_acquisition import data_set, images_Dataset,mix_dataset
from models import Big_model
import numpy as np
...
if __name__ == "__main__":
    ...
    # train_dataset=mix_dataset(train[:500000],train2[:500000],y_train[:500000],device,256)
    # train_dataloader=DataLoader(train_dataset,batch_size=39,shuffle=True,num_workers=4)

    # val_dataset=mix_dataset(val,val2,y_val,device,256)
    # val_dataloader=DataLoader(val_dataset,batch_size=39,shuffle=True,num_workers=4)

    # test_dataset=mix_dataset(test[:5000],test2,y_test[:5000],device,256)
    # test_dataloader=DataLoader(test_dataset,batch_size=64,shuffle=True)
    
    train_dataset=images_Dataset(train1[:500000],y_train[:500000],device,256)
    train_dataloader=DataLoader(train_dataset,batch_size=39,shuffle=True,num_workers=4)

    val_dataset=images_Dataset(val1,y_val,device,256)
    val_dataloader=DataLoader(val_dataset,batch_size=39,shuffle=True,num_workers=4)

    test_dataset=images_Dataset(test1[:3000],y_test[:3000],device,256)
    test_dataloader=DataLoader(test_dataset,batch_size=64,shuffle=True)
    ...
    if retrain:
        ...
        model = Big_model(route_encoder,device,512).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_accuracy_value >= best:
            ...
            torch.save(checkpoint, 'Models/Classification_Models/Best_Classification_model_b0_512_new_con_embeddings.pth')
        checkpoint = {
                    "model_state_dict": model.state_dict(),
                    "optimizer_state_dict": optimizer.state_dict(),
                    "encoder_path": model.path_to_encoder,
        ...
        torch.save(checkpoint, 'Models/Classification_Models/Last_Classification_model_b0_512_new_con_embeddings.pth')
