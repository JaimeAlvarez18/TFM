...
if __name__ == "__main__":
    ...
    CLASSES=5
    LOSS="SupConLoss"
    path_embeddings=f'Models/Embeddings/embeddings_128_2048_sd1_4_sd1_5_BigGan_SupConLoss.npz'
    ...
    embs,labels=embs1,labels1
    # embs,_,labels,_= train_test_split(embs1,labels1,train_size=0.99,stratify=labels1,random_state=39)
    print(f'Después de downsample {np.unique(labels,return_counts=True)}')
    
    knn=KNeighborsClassifier(n_neighbors=11,n_jobs=6)
    knn.fit(embs,labels)
