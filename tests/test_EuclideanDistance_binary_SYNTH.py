...
from utils.data_acquisition import data_set,test_dataset,data_set_with_nature,data_set_binary_with_nature
import torch
...
if __name__ == "__main__":
    ...
    CLASSES=8
    LOSS="SupConLoss"
    path_embeddings=f'Models/Embeddings/embeddings_{EMBEDDING_SIZE}_{BATCH_SIZE}_SYNTH_{LOSS}.npz'
    ...
    for i in range(len(all_test)):
        ...
        for image1, label in tqdm(test_dataloader, desc=f"Classifying {index + 1}/{len(test_dataloader)}"):
            ...
            with torch.no_grad():  # Disable gradient computation
                for i in range(embeddings.shape[0]):  # Iterate over each image in the batch
                    ...
                    preds=torch.softmax(torch.tensor(euclidean_distance),0)
            ...
            all_probas.extend(batched_preds)
        ...
        print(accuracy)
        print(roc_auc_score(all_labels,np.array(all_probas)[:,0]))
